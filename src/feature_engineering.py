...
def feature_engineering_training(df_in):
    ...
    if 'ORGANIZATION_TYPE' not in cat_cols_to_drop:
        ...
        with open("aux_files/organization_feature_json.json", "w") as json_file:
    ...
    df_in["CREDIT_INCOME"] = (1 + df_in["AMT_CREDIT"]) / (1 + df_in["AMT_INCOME_TOTAL"])
    df_in["ANNUITY_INCOME"] = (1 + df_in["AMT_ANNUITY"]) / (1 + df_in["AMT_INCOME_TOTAL"])
    df_in["CREDIT_GOODS_PRICE"] = (1 + df_in["AMT_CREDIT"]) / (1 + df_in["AMT_GOODS_PRICE"])
    df_in["GOODS_PRICE_INCOME"] = (1 + df_in["AMT_GOODS_PRICE"]) / (1 + df_in["AMT_INCOME_TOTAL"])
    df_in["EXTS_2_3_DIFF"] = df_in["EXT_SOURCE_2"] - df_in["EXT_SOURCE_3"]
    df_in["EXTS_2_3_MEAN"] = (df_in["EXT_SOURCE_2"] + df_in["EXT_SOURCE_3"]) / 2
    df_in["EXTS_2_3_SUM"] = df_in["EXT_SOURCE_2"] + df_in["EXT_SOURCE_3"]
...
def feature_engineering_production(df_in):
    ...
    if 'ORGANIZATION_TYPE' not in cat_cols_to_drop:
        ...
        organization_feature_json = json.load(open('aux_files/organization_feature_json.json'))
